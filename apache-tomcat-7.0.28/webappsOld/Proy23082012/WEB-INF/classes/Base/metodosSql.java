...
public class metodosSql extends ConexionMySql {
	...
	public int cerrarMes(String usuario,int mes) {
		int status=0;
		String auxdni=null;
		String periodo=damePeriodo(mes, Integer.parseInt(dameAnio()), "fecha");
		...
		if(estaUsuarioEnMesesEstado(usuario, Integer.parseInt(dameAnio()))){
			
		insertarOmodif("update MesesEstado set "+dameNroTeDoyMes(mes)+" ='CERRADO' where usuario= '"+usuario+"' and ANIO= "+Integer.parseInt(dameAnio()));
